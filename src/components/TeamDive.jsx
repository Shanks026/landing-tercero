...
const checks = [
  ...
  "Contextual comments & @mentions — loop teammates in before a client sees a draft",
  "Tasks assigned to teammates, tracked by status and due date",
  "Notion-style notes — public to the team, or kept private to you",
  "In-app notifications keep everyone automatically in the loop",
];
        ...
        <div className="grid grid-cols-2 max-md:grid-cols-1 gap-24 max-md:gap-10 items-start">
          <Reveal>
            ...
            <p className="text-base text-muted-foreground leading-[1.7] font-normal max-w-[500px]">
              Assign work by role, loop your team in with comments and
              @mentions before a client ever sees a draft, and keep notes
              scoped to the right audience — your whole team, or just you.
            </p>
          ...
          <Reveal delay={0.16}>
            <div className="flex flex-col items-center max-w-[360px] mx-auto md:mx-0">
              {taskImages.map((src, i) => (
                <img
                  key={i}
                  src={src}
                  alt="Task card"
                  className={`w-full h-auto shadow-xl rounded-lg ${i > 0 ? "-mt-10" : ""} ${
